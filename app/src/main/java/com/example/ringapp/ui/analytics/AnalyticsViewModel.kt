...
data class PersonalAnalyticsState(val habit: HabitEntity? = null, val progress: List<HabitProgressEntity> = emptyList(), val transactions: List<PointTransactionEntity> = emptyList(), val loading: Boolean = true, val error: String? = null)
...
class PersonalAnalyticsViewModel @Inject constructor(
    ...
    val state: StateFlow<PersonalAnalyticsState> = combine(observeDetail(habitId, 365), observeTransactions(habitId)) { detail, transactions -> PersonalAnalyticsState(detail?.habit, detail?.progress.orEmpty(), transactions, false) }
...
fun aggregateDaily(habits: List<HabitEntity>, progress: List<HabitProgressEntity>, date: LocalDate): AnalyticsPoint? {
    ...
    val records = active.mapNotNull { habit -> progress.firstOrNull { it.habitId == habit.id && it.date == startOfDay(date) } }
    val score = if (records.isEmpty()) 0 else records.map { it.percentage }.average().toInt()
    return AnalyticsPoint(date, score, records.sumOf { it.actual }, active.sumOf { it.target }, records.all { it.completed } && records.isNotEmpty())
}
